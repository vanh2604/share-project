...
import {getProjectsOfUserQuery} from "../grapql";
...
const queryString = gql`${getProjectsOfUserQuery}`

const RelatedProjects = ({ userId, projectId }: Props) => {
    const {loading, error, data : result} = useQuery(queryString,{
        variables: {id: userId}
    })
        ...
        <section className="flex flex-col mt-32 w-full">
            ...
            <div className="related_projects-grid">
                {filteredProjects?.map(({ node }: { node: ProjectInterface },index) => (
                    <div key={index} className="flexCenter related_project-card drop-shadow-card">
